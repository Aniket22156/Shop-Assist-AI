...
def moderation_check(user_input):
    """
    Checks user input for potentially harmful content using OpenAI's moderation API.

    Returns "Flagged" if the input is flagged, "Not Flagged" otherwise.
    Returns None if there's an error with the API call.
    """
    try:
        response = openai.moderations.create(input=user_input, model="omni-moderation-latest")  # Model specified here
